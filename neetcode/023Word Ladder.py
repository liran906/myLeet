...
# O((n ^ 2) * m) time and O(n ^ 2) space
# where n is the number of words and m is the length of the word
class Solution:
    def ladderLength(self, beginWord, endWord, wordList):
        # build graph
        wordList.append(beginWord)
        wgraph = {i:[] for i in wordList}

        # build connections
        for w1 in wordList:
            for w2 in wordList:
                if w1 == w2 or w2 in wgraph[w1] or w1 in wgraph[w2]:
                    continue
                for i in range(len(w1)):
                    w1_blank = w1[:i] + w1[i+1:]
                    w2_blank = w2[:i] + w2[i+1:]
                    if w1_blank == w2_blank:
                        wgraph[w1].append(w2)
                        wgraph[w2].append(w1)
        
        # bfs
        wqueue = [[beginWord, None]]
        searched = []
        while wqueue:
            curr = wqueue.pop(0)
            if curr[0] == endWord:
                break
            searched.append(curr[0])
            for w in wgraph[curr[0]]:
                if w not in searched:
                    wqueue.append([w, curr])
...
class Solution:
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        for w1 in wordList:
            for i in range(len(w1)):
                bucket = w1[:i] + '_' + w1[i+1:]
                if bucket in bkt:
                    bkt[bucket].append(w1)
                else:
                    bkt[bucket] = [w1]
        
        # build connections: this is O(m * n * k^2) time, k is number of word in each bucket. Still not ideal.
        for wlist in bkt.values():
            for i in range(len(wlist)):
                for j in range(i + 1, len(wlist)):
                    wgraph[wlist[i]].append(wlist[j])
                    wgraph[wlist[j]].append(wlist[i])
...
# O((m ^ 2) * n) time and O(n * m) space
# where n is the number of words and m is the length of the word
# Instead of building and storing Graph, just kept the Bucket, and used findNeighbor method to return neighbors.
class Solution:
    def findNeighbor(self, word, bkt):
        neighbor = set()
        for i in range(len(word)):
            w_blank = word[:i] + '_' + word[i+1:]
            for w in bkt[w_blank]:
                neighbor.add(w)
        neighbor.remove(word)
        return neighbor
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        # bfs
        wqueue = [[beginWord, None]]
        searched = set() # this should be set. ALOT FASTER than list.
        while wqueue:
            curr = wqueue.pop(0)
            if curr[0] == endWord:
                break
            searched.add(curr[0])
            neighbors = self.findNeighbor(curr[0], bkt)
            for w in neighbors:
                if w not in searched:
                    wqueue.append([w, curr])
...
res = s.ladderLength('sand', 'acne', wl)
print(res)